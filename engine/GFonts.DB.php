...
class GFontsDB {
        ...
        while (!$finish) {
            ...
            foreach ($usedInPosts as $name => $value) {
                if (isset($fontIdByName[$name])) {
                    ...
                    GFontsDB::InstallFontUsedIn($name, $value, in_array($name, $gfonts));
                }
            }
    ...
    static public function RemoveFontFromPost($font, $id) {
        ...
        $wpdb->query($sql);
        $sql = $wpdb->prepare("UPDATE {$wpdb->prefix}gf_fontlist SET used_in_posts = used_in_posts - 1 WHERE id = %d", $fontId);
        $wpdb->query($sql);
    ...
    static public function GetOrInstallFontByName($name, $gfont) {
        ...
        $id = $wpdb->get_var($sql);
        if ($id == null) {
            $sql = $wpdb->prepare("INSERT INTO {$wpdb->prefix}gf_fontlist(name, used_in_posts, gfont, installed) VALUES(%s, 0, %d, 1)", $name, $gfont);
            $wpdb->query($sql);
